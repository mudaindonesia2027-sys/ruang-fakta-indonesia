...
function parseStatus(value: string | null): ArticleStatus | undefined {
  ...
  if (Object.values(ArticleStatus).includes(value as ArticleStatus)) return value as ArticleStatus;
  return undefined;
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (categoryParam && categoryParam.trim()) {
      ...
      where.category = {
        is: {
          OR: [{ id: categoryValue }, { slug: categoryValue }],
        },
      };
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!allowedRoles.includes(user.role)) return NextResponse.json({ success: false, error: "Forbidden" }, { status: 403 });
    ...
    const status = Object.values(ArticleStatus).includes(requestedStatus as ArticleStatus)
      ? requestedStatus as ArticleStatus
      : ArticleStatus.DRAFT;

    const slug = await uniqueSlug(
      typeof body.slug === "string" && body.slug.trim() ? body.slug : title,
      async candidate => Boolean(await db.article.findUnique({ where: { slug: candidate } }))
    );
    ...
    if (categoryInput) {
      ...
      if (existingCategory) {
        ...
        const categorySlug = await uniqueSlug(
          categoryInput,
          async candidate => Boolean(await db.category.findUnique({ where: { slug: candidate } }))
        );
    ...
    await audit({ action: "ARTICLE_CREATED", entity: "Article", entityId: article.id, actorId: user.id });
