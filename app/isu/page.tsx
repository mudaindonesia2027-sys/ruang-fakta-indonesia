...
export default async function IsuPage({ searchParams }: { searchParams?: Promise<{ q?: string; wilayah?: string; status?: string }> }) {
  ...
  return <PublicShell><main className="public-page">
    ...
    <section className="container content-section">
      <form className="public-filters" method="get"><label><span>Cari isu</span><input name="q" defaultValue={q} placeholder="Contoh: pendidikan, jalan, harga…" /></label><label><span>Wilayah</span><input name="wilayah" defaultValue={wilayah} placeholder="Provinsi / kabupaten / kecamatan" /></label><label><span>Status</span><select name="status" defaultValue={selectedStatus}><option value="">Semua status</option>{allowedStatuses.map((value) => <option value={value} key={value}>{statusLabel(value)}</option>)}</select></label><button className="button-primary" type="submit">Terapkan</button>{(q || wilayah || selectedStatus) && <Link className="filter-reset" href="/isu">Reset</Link>}</form>
